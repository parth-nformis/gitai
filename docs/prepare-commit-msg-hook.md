...
`gitai commit-msg enable` and backs out with `gitai commit-msg disable`,
so the same installed hook serves every repo while each repo decides for
itself whether commit messages use AI. This keeps the hook as shared
...
sequenceDiagram
    ...
    H->>A: gitai -commit-msg-file <msgfile>
...
| Piece | Shape |
|---|---|
| `gitai hook install` | Subcommand: writes a small shell script to `.git/hooks/prepare-commit-msg`, `chmod +x`, per repository |
| Hook script | `[ -n "$2" ] && exit 0; [ -s "$1" ] && exit 0; gitai -commit-msg-file "$1" \|\| exit 0` — failures must never block a commit |
| `gitai -commit-msg-file <file>` | New mode (internal): `Commit` handler + `git.StagedDiff` + write result to the file instead of stdout (no decorative banners, no auto-commit) |
| Per-repo toggle | `gitai commit-msg enable` / `disable` write or remove the marker file `<git-dir>/gitai/commitmsg`; `-commit-msg-file` exits 0 when the marker is absent |
| Empty staged diff | Hook exits silently — normal `git commit` flow continues |

## Safety properties

- Never modifies the index or working tree.
- Off by default: a repo gets no AI prefill until `gitai commit-msg
  enable` is run in it; the state is a per-repo marker in the repo's
  `.git/`, not git config.
...
guards plus calling `gitai -commit-msg-file`), and the on/off decision
happens inside `gitai -commit-msg-file`, which reads the marker via the
repo's git dir.
...
`gitai -commit-msg-file ... 2>/dev/null`. git keeps the hook's stderr
attached to
