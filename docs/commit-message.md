...
flowchart TD
    ...
    B --> C["Stage 1 — per chunk:<br/>client.Generate (thinking OFF)<br/>prompt names the chunk's files"]
    C --> D["summaries: one 1-2 sentence line per chunk"]
    D --> E["Stage 2:<br/>client.Generate (thinking = user setting)<br/>prompt = all summaries + prepared.Summary"]
...
| Thinking off in Stage 1 | Mechanical summarization × N chunks — thinking would multiply latency/cost for no gain |
